...
class Portfolio:
    ...
    def get_estimation_period_data(self):
        ...
        start_estimation = self.start_of_week[0:n-4]
        end_estimation = self.start_of_week[4:n] 
        start_deploy = self.start_of_week[4:n]
        end_deploy = self.start_of_week[5:]
        for start_e, end_e, start_d, end_d in zip(start_estimation, end_estimation, start_deploy, end_deploy):
...
#             print(df_adj_close[start_e:end_e].iloc[:-1,:])
#             print(df_adj_close[start_d:end_d].iloc[:-1,:])
